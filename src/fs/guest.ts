...
export function useGuestFs(connection: Connection, rpc: BirpcReturn<HostFunctions, GuestFunctions>, hostId: string, canEdit: () => boolean = () => true) {
  const { fileChanged, useSetActiveProvider } = useFsProvider()
  let lastPermissionWarning = 0
  function assertCanEdit() {
    if (canEdit()) {
      return true
    }
    if (Date.now() - lastPermissionWarning > 3000) {
      lastPermissionWarning = Date.now()
      window.showWarningMessage('You have view-only access in this CodeTogether session.')
    }
    return false
  }
  function noPermission() {
    assertCanEdit()
    throw FileSystemError.NoPermissions('You have view-only access in this CodeTogether session.')
  }
