...
export function useHostFs(connection: Connection, sessionDoc?: Y.Doc) {
  ...
  recv((update, peerId, meta) => {
    const permissions = sessionDoc?.getMap<{ canEdit: boolean }>('permissions')
    if (permissions?.get(peerId)?.canEdit === false) {
      return
    }
