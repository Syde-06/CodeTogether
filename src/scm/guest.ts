...
export function useGuestScm(doc: Y.Doc, _rpc: BirpcReturn<HostFunctions, GuestFunctions>) {
  ...
  useCommands({
    'codetogether.scm.openFile': async (resource, ...additionalResources) => {
      let urisToOpen = []
      if (resource) {
    ...
    'codetogether.scm.cleanAll': async (group: SourceControlResourceGroup) => {
    ...
    'codetogether.scm.revertChange': async (..._args) => {
...
function useScmRepo(uri: string, repo: ScmRepo) {
  ...
  const sc = useDisposable(scm.createSourceControl('codetogether-scm', meta.label, Uri.parse(meta.rootUri)))
...
class ScmResourceState implements SourceControlResourceState {
  ...
  constructor(change: ScmChange, meta: ScmGroupMeta, repoUri: string) {
    ...
    this.command = {
      command: 'codetogether.scm.openChange',
      title: 'Open',
