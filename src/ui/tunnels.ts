...
export const useTunnelsTree = defineService(() => {
  useVscodeContext('codetogether:supportsTunnels', true)
  ...
  useTreeView(
    ...
    computed(() => [...hostByMe.value, ...connected.value, ...available.value].map((tunnel) => {
      ...
      if (serving) {
        ...
        command = {
          command: 'codetogether.copySharedServerLocalURL',
        ...
        command = {
          command: 'codetogether.connectToSharedServer',
  ...
  useCommands({
    'codetogether.shareServer': async () => {
      if (!tunnels.value) {
    ...
    'codetogether.stopSharingServer': async (item?: any) => {
      if (!tunnels.value) {
        window.showWarningMessage('No active session or not supported.')
        return
      }
    ...
    'codetogether.connectToSharedServer': async (item?: any) => {
      if (!tunnels.value) {
        window.showWarningMessage('No active session or not supported.')
        return
      }
      ...
      const port = await window.withProgress({
        location: { viewId: 'codetogether.tunnels' },
    ...
    'codetogether.disconnectFromSharedServer': async (item?: any) => {
      if (!tunnels.value) {
        window.showWarningMessage('No active session or not supported.')
        return
      }
    ...
    'codetogether.copySharedServerLocalURL': async (item?: any) => {
      if (!tunnels.value) {
        window.showWarningMessage('No active session or not supported.')
        return
      }
