...
function useFollowing(map: ComputedRef<Y.Map<SelectionInfo> | undefined>, mapVersion: Ref<number>) {
  ...
  useCommands({
    'codetogether.focusParticipant': async (target?: string | any) => {
      const peerId = typeof target === 'string'
        ? target
        : target?.treeItem?.peerId || await pickPeerId()
      if (peerId) {
    ...
    'codetogether.followParticipant': async (item: any) => {
      const peerId = item?.treeItem?.peerId || await pickPeerId()
      if (peerId) {
        following.value = peerId
      }
    },
    'codetogether.unfollowParticipant': () => {
