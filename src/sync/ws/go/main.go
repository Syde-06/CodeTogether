...
func main() {
	...
	pflag.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, strings.Join([]string{"",
			"Examples:",
			"  codetogether-ws-server",
			"  codetogether-ws-server -p 9000",
			"  codetogether-ws-server --port 9000 --hostname 0.0.0.0",
		}, "\n"))
	}
	pflag.Parse()
...
func handleConnection(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" && r.URL.Path == "/" {
		...
		msg := fmt.Sprintf("CodeTogether WebSocket Signaling Server. %d active room(s).", roomCount)
